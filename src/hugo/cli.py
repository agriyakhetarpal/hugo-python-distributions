...
def _editable_hugo_executable() -> Path | None:
    """Resolve the bundled binary from meson-python's editable install plan."""
    for finder in sys.meta_path:
        ...
        for targets in plan.values():
            for source, target in targets.items():
                ...
                if destination.parts[-2:] == ("binaries", HUGO_EXECUTABLE):
                    return Path(source)
...
def _hugo_executable():
    binary = Path(sysconfig.get_path("scripts"), "binaries", HUGO_EXECUTABLE)
    if binary.is_file():
